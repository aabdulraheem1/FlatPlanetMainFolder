...
import polars as pl
import pandas as pd  # Keep for specific operations where needed
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            with transaction.atomic():
                ...
                forecast_df_pd = pd.DataFrame(list(forecast_queryset))
                ...
                # Extract warehouse location (last 4 characters after delimiter) using polars
                forecast_df = forecast_df.with_columns([
                    pl.col('Location').map_elements(
                        self.extract_location_from_forecast, 
                ...
                processed_df = pl.DataFrame(processed_rows)
                ...
                processed_df = processed_df.with_columns([
                    pl.col('shipping_date').map_elements(
                        lambda x: x.replace(day=1), return_dtype=pl.Date
                    ).alias('shipping_year_month')
                ])

                # Aggregate
                aggregated_df = processed_df.group_by([
                    'Product', 'warehouse_location', 'shipping_year_month', 'production_site'
                ]).agg([
                    pl.col('Qty').sum(),
                    pl.col('shipping_date').first(),  # Keep one shipping date per group
                    pl.col('Forecast_Region').first(),
                    pl.col('Customer_code').first()
                ])

                # Sort chronologically
                aggregated_df = aggregated_df.sort(['Product', 'warehouse_location', 'shipping_year_month'])
        ...
        # STEP 5: Inventory deduction and replenishment calculation (OPTIMIZED)
        ...
        # OPTIMIZATION: Load safety stock data in bulk and convert to polars for faster lookups
        safety_stock_list = list(MasterDataSafetyStocks.objects.filter(version=scenario).values(
            'Plant', 'PartNum', 'MinimumQty', 'SafetyQty'
        ))
        safety_stock_map = {}
        for ss in safety_stock_list:
        ...
        # OPTIMIZATION: Use vectorized aggregation for inventory
        location_inventory = defaultdict(lambda: defaultdict(lambda: {'onhand': 0, 'intransit': 0}))
        
        if len(inventory_df) > 0:
            # Group and aggregate inventory in polars for better performance
            inventory_agg = inventory_df.group_by(['site__SiteName', 'product']).agg([
                pl.col('onhandstock_qty').sum().alias('total_onhand'),
                pl.col('intransitstock_qty').sum().alias('total_intransit')
            ])
            
            # Convert aggregated results to dictionary
            for row in inventory_agg.iter_rows(named=True):
                location_key = row['site__SiteName']
                product_key = row['product']
                location_inventory[location_key][product_key]['onhand'] = row['total_onhand'] or 0
                location_inventory[location_key][product_key]['intransit'] = row['total_intransit'] or 0
        ...
        unique_months = sorted(aggregated_df['shipping_year_month'].unique().to_list())
        
        # OPTIMIZATION: Process in batches to reduce memory usage
        batch_size = 1000
        total_processed = 0
        
        for month in unique_months:
            self.stdout.write(f"Processing month: {month.strftime('%Y-%m')}")
            month_data = aggregated_df.filter(pl.col('shipping_year_month') == month)
            
            # Process month data in batches
            month_rows = list(month_data.iter_rows(named=True))
            for i in range(0, len(month_rows), batch_size):
                batch = month_rows[i:i + batch_size]
                
                for row in batch:                # STEP 7: Calculate pouring dates for replenishment records
                ...
                if len(production_df) > 0:
                    ...
                    production_df = production_df.with_columns([
                        pl.col('pouring_date').map_elements(
                            lambda x: x.replace(day=1), return_dtype=pl.Date
                        ).alias('pouring_year_month')
                    ])

                    production_aggregated = production_df.group_by([
                        'site', 'product', 'pouring_year_month'
                    ]).agg([
                        pl.col('replenishment_qty').sum(),
                        pl.col('pouring_date').first()
                    ])
                    ...
                    for row in inventory_df.iter_rows(named=True):
                        site_key = row['site__SiteName']
                        product_key = row['product']
                        production_inventory[site_key][product_key]['onhand'] += row['onhandstock_qty'] or 0
                        production_inventory[site_key][product_key]['intransit'] += row['intransitstock_qty'] or 0
                        production_inventory[site_key][product_key]['wip'] += row['wip_stock_qty'] or 0

                    # Calculate production requirements
                    production_instances = []
                    
                    for row in production_aggregated.iter_rows(named=True):
                        site = row['site']
                        product = row['product']
